...
-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- - 华丽の分界线-- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- -- --
  ...
  一、关键字
    ...
    1、instanceof 检测变量是否是属于某个类型的对象 只对引用变量有效。
    2、call和apply 第一个参数是this,call和apply的区别在于参数的传递不同 可以传递不同的this调用不属于其本身的方法
      call(thisArgs,arg1,arg2,arg3...);
      apply(thisArgs,[arg1,arg2,arg3...]);
    3、delete 删除对象的属性 但不能删除对象的方法以及对基本类型是无效的
    4、arguments 函数内部用来接收参数的伪数组 其本质是对象的属性,让后长得像数组.
    5、callee 指向函数本体
    6、this 指向调用当前函数或者方法的对象.
